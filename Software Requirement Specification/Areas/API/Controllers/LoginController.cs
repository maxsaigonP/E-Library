...
namespace Software_Requirement_Specification.Areas.API.Controllers
{
    ...
    public class LoginController : ControllerBase
    {
        ...
        {
            var email = await _context.TaiKhoan.Where(t => t.NguoiDung.Email==_to&&t.NguoiDung.SoDienThoai==sdt).ToListAsync();
            if (email.Count > 0)
            {
        ...
        public async Task<string> DoiMatKhau(string pass, string rePass)
        {
            ...
            if(pass==rePass)
            {
                var tk = await _context.TaiKhoan.Where(t => t.NguoiDung.Email == HttpContext.Session.GetString("Gmail")).FirstOrDefaultAsync();
                if(tk!=null)
                {
                    tk.MatKhau = pass;
                }else
