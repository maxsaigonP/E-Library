...
namespace Software_Requirement_Specification.Areas.API.Controllers
{
    ...
    public class DeThisController : ControllerBase
    {
        ...
        public DeThisController(ApplicationDbContext context, IWebHostEnvironment webHostEnvironment)
        {
            _context = context;
        ...
        public async Task<ActionResult<DeThi>> DeThi(int id)
        {
            ...
            var result = (from a in _context.DeThi
                          ...
                          select new { 
                              ...
                              TenDeThi=a.TenDeThi,
                              HinhThuc=a.HinhThuc,
                              GiaoVienDaoTao=a.TaiKhoan.UserName
                          }).ToList();
        ...
        public async Task<ActionResult<IEnumerable<DeThi>>> LocDeThi(int? tt, string nk, int mh, string gv)
        {
            ...
            if (gv != "" && mh != 0)
            {
                result = await _context.DeThi.Where(t => t.TaiKhoanId == gv && t.MonHocId == mh && t.NienKhoa.Equals(nk)).ToListAsync();
            }
            else
            if (mh != 0)
            {
            ...
            if (gv != "")
            {
                result = await _context.DeThi.Where(t => t.TaiKhoanId == gv && t.NienKhoa.Equals(nk)).ToListAsync();
            }
        ...
        //    deThi.TepId = tep.Id;
        //    deThi.TinhTrang = false;
        //    deThi.TaiKhoanId= HttpContext.Session.GetString("Nd");
        //    _context.DeThi.Add(deThi);
        //    await _context.SaveChangesAsync();
        //    //
        //    var tb = new ThongBao();
        //    tb.LoaiThongBao = true;
        //    tb.TaiKhoanId = HttpContext.Session.GetString("Nd"); //login
        ...
        public async Task<IActionResult> XoaDeThi(int id)
        {
            ...
            var tb = new ThongBao();
            tb.LoaiThongBao = true;
            tb.TaiKhoanId = HttpContext.Session.GetString("Nd");
