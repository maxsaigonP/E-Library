...
namespace Software_Requirement_Specification.Areas.API.Controllers
{
    ...
    public class TaiLieuxController : ControllerBase
    {
        ...
        public TaiLieuxController(ApplicationDbContext context, IWebHostEnvironment webHostEnvironment)
        {
            _context = context;
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<TaiLieu>>> LocTepRiengTu(int ?tt, string gv, int mh)
        {
            ...
            if (gv!=""&&mh!=0)
            {
                result = await _context.TaiLieu.Where(t => t.TaiKhoanId == gv&& t.MonHocId == mh&& t.LoaiTaiLieu=="Tệp riêng tư").ToListAsync();
            }else
            if(mh!=0)
            {
            ...
            if (gv != "")
            {
                result = await _context.TaiLieu.Where(t => t.TaiKhoanId == gv && t.LoaiTaiLieu == "Tệp riêng tư").ToListAsync();
            }
        ...
        public async Task<ActionResult> ThemBaiGiang([FromBody] TaiLieu taiLieu )
        {
        ...
        public async Task<ActionResult> ThemTaiNguyen([FromForm] TaiLieu taiLieu)
        {
        ...
        public async Task<ActionResult> ThemVaoMonHoc(int taiLieu,string tieuDe,int monHoc, int lopHoc)
        {
            ...
            tl.MonHocId = monHoc;
            tl.TaiKhoanId = tlieu.TaiKhoanId;
            tl.TinhTrang = true;
